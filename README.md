...
- **Verifiable verdicts.** LeaseClear's citations are extracted from every `lease_qa` answer and carried through the responsibility check, the draft, the approval card, and the run trace, so the human can see which clause the verdict rests on. (Click-through to LeaseClear itself is on the list below.)
...
- **Clickable citations.** Let the human click a citation in the approval card and land in LeaseClear on the same question the agent asked, to verify the verdict themselves.
